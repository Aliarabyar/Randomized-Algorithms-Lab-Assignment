...
int ds_union(ds_forest *f, int x, int y) {
    return ds_link(f, find(f, x), find(f, y));
}

int testsDisjointSet() {
    int nodes = 12;
    ds_forest *f = makeForest(nodes);
	...
	ds_link(f, i, i + 1);
	assert(f->p[i] == i + 1); assert(f->p[i + 1] == i + 1); assert(f->rank[i] == 0); assert(f->rank[i + 1] == 1);

	// find (rank 1 and 0) - no side effects
	assert(find(f, i) == i + 1); assert(find(f, i + 1) == i + 1);
    }
    // union (root & root)
    ds_union(f, 1, 3);
    assert(f->p[0] == 1);  assert(f->p[1] == 3); assert(f->p[2] == 3); assert(f->p[3] == 3);  assert(f->rank[3] == 2);

    // union (child & child)
    ds_union(f, 4, 6);
    assert(f->p[4] == 5);  assert(f->p[5] == 7);  assert(f->p[6] == 7);  assert(f->p[7] == 7); assert(f->rank[7] == 2);

    // union (root & child)
    ds_union(f, 9, 10);
    assert(f->p[8] == 9);   assert(f->p[9] == 11); assert(f->p[10] == 11);  assert(f->p[11] == 11);  assert(f->rank[11] == 2);
